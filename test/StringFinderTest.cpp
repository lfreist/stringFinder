...
TEST(StringFinderTest, testConstructor) {
  ...
  {  // default argument values
    ...
    ASSERT_NE(string_finder._fileDataMap._find_(filepath), string_finder._fileDataMap.end());
...
TEST(StringFinderTest, testReadFile) {
  ...
  ASSERT_NE(string_finder._fileDataMap._find_(filepath), string_finder._fileDataMap.end());
  ASSERT_NE(string_finder._fileDataMap._find_(filepath2), string_finder._fileDataMap.end());
...
TEST(StringDataTest, testFind) {
  ...
  {
    vector<const string *> result = string_finder._find_("NotInFile", true);
    ASSERT_EQ(result.size(), 0);
    result = string_finder._find_("NotInFile", false);
    ASSERT_EQ(result.size(), 0);
  }
  {  // searching uppercase "E"
    vector<const string *> result = string_finder._find_("E", true);
    ...
    result = string_finder._find_("E", false);
  ...
  {
    vector<const string *> result = string_finder._find_("EurEka", true);
    ASSERT_EQ(result.size(), 1);
    ASSERT_STREQ(result[0]->c_str(), "LinE with word EurEka with uppEr casE E");
    result = string_finder._find_("EurEka", false);
    ASSERT_EQ(result.size(), 2);
