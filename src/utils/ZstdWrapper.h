...
class ZstdWrapper {
 public:
  ...
  static std::vector<char> compress(void* src, size_t numBytes, int compressionLevel = 3) {
  ...
  template <typename T>
  static size_t decompressToBuffer(const char* src, size_t numBytes, T* buffer, size_t bufferCapacity) {
    auto decompressedSize = ZSTD_decompress(buffer, bufferCapacity, const_cast<char*>(src), numBytes);
    if (ZSTD_isError(decompressedSize)) {
      exit(26);
    }
